...
from ceph_volume import decorators, process, allow_loop_devices
...
class List(object):
    ...
    def exclude_invalid_loop_devices(self, devices: _List[Dict[str, str]]) -> _List[str]:
        return [
            dev
            for dev in devices
            if (dev_name := dev["NAME"]) and (not dev_name.startswith("/dev/loop") or os.path.exists(dev_name))
        ]
